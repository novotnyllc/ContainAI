...
1. **Direct Execution via Seccomp** – The container entrypoint executes the vendor binary via `agentcli-exec`, which installs a seccomp filter that traps `execve`/`execveat` syscalls. When the vendor binary (or any dependency) attempts to spawn a subprocess, the kernel pauses the thread and notifies `agent-task-runnerd`. The daemon validates the request against policy, logs the intent, and then spawns the sandboxed workload.
2. **Seccomp user-notification path (backstop)** – This mechanism serves as both the primary interception point and the backstop. The CLI itself is unaware this interception is happening, but it guarantees that even code paths we do not control hit the policy engine.

Wrappers are no longer used. The `agentcli-exec` shim ensures that the vendor binary runs with the necessary security hooks from the moment it starts. All enforcement occurs in the runner/seccomp layer rather than inside vendor code.
