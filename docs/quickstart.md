...
> **Note:** `cai install --yes` also wires shell integration via `~/.config/containai/profile.d/containai.sh` for PATH and completion (`cai`, `containai-docker`, and `docker-containai` if you create that alias).
