...
1. **Install** - Clone the repo and install `cai`:
   ...
   `cai install --yes` wires shell integration using `~/.config/containai/profile.d/containai.sh`, including completion for `cai`, `containai-docker`, and `docker-containai` if you create that alias.
