...
1. **Install** - Clone the repo and install `cai`:
   ...
   `cai install --yes` writes `~/.config/containai/profile.d/containai.sh` and adds a shell-profile hook (`~/.bash_profile`, `~/.bashrc`, or `~/.zshrc`) that sources profile.d scripts automatically, including completion for `cai`, `containai-docker`, and `docker-containai` if you create that alias.
