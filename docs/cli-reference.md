...
**Source of truth**: The native CLI implementation (`src/cai/Operations/Facade/CaiOperationsService.cs`, `src/cai/Importing/Facade/CaiImportService.cs`, `src/cai/ConfigManifest/Service/CaiConfigManifestService.cs`, and `src/ContainAI.Cli/`) is authoritative. The `cai --help` output shows commonly-used options; this documentation is comprehensive and includes advanced flags not shown in help output.
...
**Verification**: Check `src/cai/Operations/Facade/CaiOperationsService.cs`, `src/cai/Importing/Facade/CaiImportService.cs`, `src/cai/ConfigManifest/Service/CaiConfigManifestService.cs`, and `src/ContainAI.Cli/RootCommandBuilder.cs` for canonical command/flag routing.
...
- Shell integration enables PATH setup plus command completion for `cai`, `containai-docker`, and `docker-containai` if you create that alias.
- `~/.config/containai/profile.d/containai.sh` is generated by `cai install` and may be overwritten on reinstall.
...
- `cai install --yes` wires the completion hook for `cai`, `containai-docker`, and `docker-containai` if you create that alias.
