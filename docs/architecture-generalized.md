...
We strip the container of ability to attack the host kernel.
  ...
  * **AppArmor:** Mandatory Access Control (`apparmor-containai-agent.profile`) restricts the container process. Even if the agent gains root, it cannot write to `/proc`, `/sys`, or load kernel modules.
