# Initial Architecture & Product Definition: CodingAgents

## 1\. Strategic Vision

**"The Safe Runtime for Autonomous Workloads."**

CodingAgents is a local-first infrastructure platform designed to host autonomous AI agents (GitHub Copilot, Codex, Claude) with **"Unrestricted" agency**—granting them shell access, network tools, and compiler toolchains—while maintaining **"Air-Gapped" security** for the host system.
...
flowchart TB
    subgraph host["HOST SYSTEM (Trusted)"]
        direction TB
        installer["Immutable Install<br/>/usr/local/share/coding-agents<br/>(Root Owned)"]
    ...
    launcher["Runtime CLI (coding-agents)<br/>• Verifies Integrity (SBOM)<br/>• Enforces AppArmor<br/>• Bootstraps Broker"]
    ...
    subgraph container["CONTAINER (Hardened)"]
        ...
        tmpfs["Session tmpfs (/run/coding-agents)<br/>• Sealed Capabilities"]
...
We strip the container of ability to attack the host kernel.
  ...
  * **Seccomp:** Custom filter (`seccomp-coding-agents.json`) blocks dangerous syscalls (`ptrace`, `mount`, `keyctl`, `bpf`).
  * **AppArmor:** Mandatory Access Control (`apparmor-coding-agents.profile`) restricts the container process. Even if the agent gains root, it cannot write to `/proc`, `/sys`, or load kernel modules.
...
| :--- | :--- | :--- |
| **Linux/macOS** | `/usr/local/share/coding-agents` | `root` |
| **Windows** | `%ProgramFiles%\CodingAgents` | `Administrators` |

### 5.2 The "Glass Box" Integrity Chain

We enforce supply chain security using standard artifacts.

1.  **Transport:** `coding-agents.tar.gz` (Signed via Sigstore). SHA-256 is the **Trust Anchor**.
...
flowchart TB
    subgraph specialized["Specialized Images"]
        direction TB
        spec["• coding-agents-copilot<br/>• coding-agents-claude"]
    end
