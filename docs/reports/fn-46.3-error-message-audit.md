...
### Pattern 1: `_cai_error` Function (Primary, Legacy Snapshot)

The legacy shell implementation used `_cai_error()` in `src/lib/core.sh:110`:
```bash
_cai_error() {
    printf '%s\n' "[ERROR] $*" >&2
}
...
- `src/lib/import.sh` (1 call, legacy file removed in .NET-native migration)
