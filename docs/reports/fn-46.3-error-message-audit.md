...
**Status:** Historical pre-.NET-native audit. Current runtime/error handling lives in `src/cai/NativeLifecycleCommandRuntime.cs` and `src/ContainAI.Cli/`.
...
### Pattern 1: `_cai_error` Function (Primary, Shell Snapshot)

The previous shell implementation used `_cai_error()` in `src/lib/core.sh:110`:
```bash
_cai_error() {
    printf '%s\n' "[ERROR] $*" >&2
}
...
- `src/lib/import.sh` (1 call, file removed in .NET-native migration)
- `src/lib/env.sh` (1 call)

### Pattern 2: Direct `echo "[ERROR]"` (Shell Snapshot)

**515 occurrences** - This is the previous shell pattern, primarily in `src/containai.sh`. Many use follow-up lines for context but not remediation.
...
| `Container was created before templates. Use --fresh to rebuild with template.` | 3 | Older container | 4 |

### Template Category: Isolation/Setup Errors

| Template | Callsites | Example | Score |
|----------|-----------|---------|-------|
| `No isolation available. Run 'cai doctor' for setup instructions.` | 9 | Setup not complete | 4 |
| `No isolation context available. Run 'cai setup' to create {ctx}.` | 6 | Context missing | 4 |
...
| Template | Callsites | Example | Score |
|----------|-----------|---------|-------|
| `Docker is not installed or not in PATH` | 8 | Docker missing | 2 |
| `No timeout command available (timeout, gtimeout, or perl required)` | 1 | Timeout utility missing | 2 |
| `Docker daemon is not accessible` | 1 | Daemon unreachable | 2 |
...
| Template | Callsites | Example | Score |
|----------|-----------|---------|-------|
| `Config file not found: {path}` | 12 | File not found | 2 |
| `Failed to parse config file: {path}` | 8 | Parse error | 2 |
| `Failed to parse config: {path}` | 10 | Parse error variant | 2 |
| `Python required to parse config: {path}` | 3 | Dependency missing | 2 |
| `Failed to determine script directory` | 9 | Internal error | 1 |
