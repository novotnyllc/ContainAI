...
# ==============================================================================
# Sync VS Code Insiders settings and extensions to Docker volume
# ==============================================================================
#
# Syncs VS Code Insiders User settings to dotnet-sandbox-vscode volume for use
# inside the Docker container. Detects host OS automatically.
#
# Usage: sync-vscode-insiders.sh [options]
#   --dry-run    Show what would be done without making changes
#   --help       Show this help message
#
# Exit codes:
#   0 - Success (or VS Code Insiders not installed)
...
# Detect host OS and set VS Code Insiders paths
    ...
    case "$os_type" in
        ...
        Linux)
            ...
            if grep -qi microsoft /proc/version 2>/dev/null; then
                ...
                if [[ -n "$win_user" ]]; then
                    VSCODE_USER_DIR="/mnt/c/Users/$win_user/AppData/Roaming/Code - Insiders/User"
                else
...
sync_extensions_list() {
    ...
    # Write extensions list to volume (in insiders subdirectory)
    printf '%s\n' "$extensions_list" | docker run --rm -i \
        -v "$VOLUME_NAME":/target \
        alpine:3.20 sh -c "
            mkdir -p /target/data-insiders
            cat > /target/data-insiders/extensions.txt
            chown -R 1000:1000 /target/data-insiders
        "
...
show_summary() {
    echo ""
    echo "================================================================"
    success "VS Code Insiders sync complete!"
    echo "================================================================"
    echo ""
...
main() {
    echo "================================================================"
    info "Syncing VS Code Insiders settings to Docker volume"
    echo "================================================================"
    echo ""
