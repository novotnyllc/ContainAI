...
test_bash_aliases_linked_assertions() {
    ...
    # Container should have it linked as ~/.bash_aliases_imported
    assert_symlink_target "/home/agent/.bash_aliases_imported" "/mnt/agent-data/shell/bash_aliases" || {
        printf '%s\n' "[DEBUG] ~/.bash_aliases_imported should symlink to /mnt/agent-data/shell/bash_aliases" >&2
        # Debug: show what the symlink actually points to
        local actual_target
        actual_target=$(exec_in_container "$SYNC_TEST_CONTAINER" readlink "/home/agent/.bash_aliases_imported" 2>/dev/null) || true
        printf '%s\n' "[DEBUG] Actual symlink target: $actual_target" >&2
        return 1
    }
...
test_inputrc_synced_assertions() {
    ...
    assert_symlink_target "/home/agent/.inputrc" "/mnt/agent-data/shell/inputrc" || {
        printf '%s\n' "[DEBUG] ~/.inputrc should symlink to /mnt/agent-data/shell/inputrc" >&2
        return 1
    }
...
test_zshrc_synced_assertions() {
    ...
    # Verify symlink in container
    assert_symlink_target "/home/agent/.zshrc" "/mnt/agent-data/shell/zshrc" || {
        printf '%s\n' "[DEBUG] ~/.zshrc should symlink to /mnt/agent-data/shell/zshrc" >&2
        return 1
    }
...
test_zprofile_synced_assertions() {
    ...
    # Verify symlink in container
    assert_symlink_target "/home/agent/.zprofile" "/mnt/agent-data/shell/zprofile" || {
        printf '%s\n' "[DEBUG] ~/.zprofile should symlink to /mnt/agent-data/shell/zprofile" >&2
        return 1
    }
...
test_ohmyzsh_custom_synced_assertions() {
    ...
    # Verify symlink in container
    assert_symlink_target "/home/agent/.oh-my-zsh/custom" "/mnt/agent-data/shell/oh-my-zsh-custom" || {
        printf '%s\n' "[DEBUG] ~/.oh-my-zsh/custom should symlink to /mnt/agent-data/shell/oh-my-zsh-custom" >&2
        return 1
    }
