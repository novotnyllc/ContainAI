...
- Added `_containai_resolve_import_exclude_priv()` function for explicit_config support

### 4. Updated tgz restore path (src/lib/import.sh)
- Added `--exclude '*.priv.*'` to tar extraction for shell/bashrc.d paths
- Uses path-specific excludes for correct filtering
...
### 8. Updated check-manifest-consistency.sh (scripts/)
- Added `p` flag to `normalize_flags()` function

### 9. Updated parse-toml.py (src/)
- Added `exclude_priv` and `import.exclude_priv` to `bool_keys` for proper boolean parsing

## Test Results
- Test 61: PASS - Normal import filters .priv. files
- Test 62: PASS - --no-excludes does NOT disable .priv. filtering (security verified)
## Changes Made
...
## Evidence
- Commits: 120c3a7 fix(review): address Codex impl-review feedback for fn-31-gib.16
- Tests:
- PRs:
