...
## Done summary
Added --reset-lima flag to cai doctor command for macOS users to reset their Lima VM and Docker context. The flag deletes the VM, removes the Docker context, and clears the template hash, with proper error handling and user confirmation.
## Evidence
- Commits: 67ce542, 78c1035, cffd15d, 6b1c1f0
- Tests: shellcheck -x src/lib/doctor.sh src/containai.sh, source src/containai.sh && cai doctor --reset-lima, source src/containai.sh && cai doctor --help
- PRs:
