...
## Done summary
Implemented hash-based Lima template change detection for macOS update. Only recreates VM when template changes; otherwise runs non-interactive apt update/upgrade in existing VM. Added --lima-recreate flag to bypass hash check.
## Evidence
- Commits: 6e64ebb, e0ce883, 76cd2e1, 46e54c8
- Tests: shellcheck -x src/lib/setup.sh src/lib/update.sh, source src/containai.sh && _cai_lima_template_hash, source src/containai.sh && cai update --help
- PRs:
