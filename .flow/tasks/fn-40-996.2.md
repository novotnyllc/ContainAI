...
## Done summary
# ACP Proxy Integration Testing Complete

Verified comprehensive integration tests for the ACP proxy covering all critical failure modes:

## Test Results: 15/15 PASS

### Tests Implemented:
1. **NDJSON framing** - Validates newline-delimited JSON (not Content-Length headers)
2. **Stdout purity** - No diagnostic output leaks, even with CONTAINAI_VERBOSE=1
3. **Initialize response** - Correct protocol negotiation
4. **Single session lifecycle** - session/new → session/prompt → session/update
5. **Multiple simultaneous sessions** - Different workspaces supported
6. **Concurrent output serialization** - No byte interleaving with rapid prompts
7. **Session ID namespacing** - Proxy-generated UUIDs, not raw agent IDs
8. **Session routing by proxySessionId** - Prompts routed to correct sessions
9. **Workspace resolution** - Subdirectories normalized to git root
10. **MCP path translation** - Host paths → container paths, relative paths preserved
11. **Session cleanup on session/end** - Session removed, subsequent prompts fail
12. **Stdin EOF graceful shutdown** - Clean exit on stdin closure
13. **Error handling - session not found** - Proper error codes
14. **Numeric JSON-RPC IDs** - Type preservation
15. **Unknown method returns error** - Method not found (-32601)

### CI Integration:
- Tests run on every PR via `.github/workflows/docker.yml`
- Builds proxy with .NET 10 preview
- Uses test mode (CAI_ACP_TEST_MODE=1, CAI_ACP_DIRECT_SPAWN=1)

### Key Files:
- `tests/integration/test-acp-proxy.sh` - Main test runner (1143 lines)
- `tests/integration/mock-acp-server` - Bash mock ACP server (117 lines)
## Evidence
- Commits:
- Tests:
- PRs:
