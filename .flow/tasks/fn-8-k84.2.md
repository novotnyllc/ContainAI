...
## Done summary
Made integration tests hermetic by adding early Docker availability guard, creating temp HOME fixture under REAL_HOME with mktemp, preserving DOCKER_CONFIG, and routing all cai import calls through hermetic helpers that override HOME to FIXTURE_HOME.
## Evidence
- Commits: 7a4d83a, c264476, 7e44050, fbd61f4
- Tests: bash -n test-sync-integration.sh (syntax check), ./test-sync-integration.sh (early guard verified - exits with FAIL when docker daemon not running)
- PRs:
