...
## Done summary
Verified that --container parameter is fully implemented for shell, run, import, export, and stop commands. All commands correctly reject --name (stop has explicit migration message, others show usage help). Links command correctly retains --name for different semantics. Mutual exclusivity checks and label-based derivation are working correctly.
## Evidence
- Commits: cc923bf, f168c65, 64d6712, bfd0fcc, 92ca4ed
- Tests: shellcheck -x src/containai.sh, shellcheck -x src/lib/*.sh, cai stop --name test (verified rejection), cai run/shell/import/export --name (verified rejection), cai links --help (verified --name retained)
- PRs:
