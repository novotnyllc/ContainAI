...
- [ ] Dockerfile: wasm-tools and wasm-tools-net9 in separate RUN commands
- [ ] Dockerfile: wasm-tools-net9 uses fail-open pattern (`|| echo`)
- [ ] csd: Checks label support before using `--label`
- [ ] csd: Falls back to image name verification if labels not supported
- [ ] csd: Chowns ALL volumes including docker-claude-sandbox-data to uid 1000
- [ ] csd: Uses base image as fallback for permission fixing if dotnet-sandbox not built
- [ ] csd: Blocking errors include raw docker output
- [ ] csd: Collision errors include expected vs actual identity
- [ ] README: Clarifies sync-plugins does NOT sync credentials
- [ ] csd: Container name fallback uses `sandbox-<dirname>` pattern
