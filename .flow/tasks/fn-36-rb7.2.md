...
## Acceptance
- [x] `cai shell --container foo` uses if exists, creates if missing
- [x] `cai run --container foo` uses if exists, creates if missing
- [ ] `cai exec --container foo` uses if exists, creates if missing (NOTE: exec command is fn-36-rb7.6)
- [x] `cai stop --container foo` errors "Container foo not found" if missing
- [x] `cai export --container foo` errors if missing
- [x] `cai import --container foo` errors if missing
- [x] Container name is saved to workspace state on successful create/use
- [x] Help text documents "uses existing or creates new" behavior
- [x] Errors guide users who expected old behavior
