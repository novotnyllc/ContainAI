...
## Done summary
Implemented ACP terminating proxy for editor integration with native AOT binary in C# .NET 10. The proxy handles ACP protocol from editors, supports multi-session with namespaced session IDs, routes messages by sessionId, and spawns containerized agents via `cai exec`. Includes build script with `cai update` integration.
## Evidence
- Commits: d407758, ce40e22, bd12da5, 088e1ae, 32e235a, ce96d99, 04f500b, 71c0eb9, f8d6be4, 98a7858
- Tests: shellcheck -x src/containai.sh, shellcheck -x src/lib/update.sh, dotnet build, src/acp-proxy/build.sh --install
- PRs:
