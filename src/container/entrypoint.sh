...
_setup_git_config() {
    ...
    # But we still need to run migration for volume paths
    if [[ -L "$dst" ]]; then
        return 0
    fi
...
# Run migration first (handles volume path upgrade)
_migrate_git_config
# Then setup (handles legacy containers without symlink)
_setup_git_config
