...
#   _cai_resolve_sysbox_download_url() - Resolve sysbox download URL (ContainAI first)
...
_cai_install_sysbox_wsl2() {
    ...
    local sysbox_source="$_CAI_SYSBOX_SOURCE"

    _cai_info "Sysbox version: $sysbox_version (source: $sysbox_source)"
    if [[ "$verbose" == "true" ]]; then
...
_cai_spacing
    ...
    # Uses shared _cai_apply_network_rules() which is platform-aware for macOS/Lima
    _cai_spacing
    _cai_step "Applying network security rules"
    if ! _cai_apply_network_rules "$dry_run"; then
        _cai_warn "Network security rules failed to apply - check output above"
...
_cai_install_sysbox_linux() {
    ...
    local download_url="$_CAI_SYSBOX_DOWNLOAD_URL"
    local latest_version="$_CAI_SYSBOX_VERSION"
    local sysbox_source="$_CAI_SYSBOX_SOURCE"
