...
'
    ...
    # Built from rewritten_entries (post --no-secrets and excludes filtering) so symlinks
    # only resolve to entries that were actually imported
    local manifest_data=""
    while IFS= read -r entry; do
        [[ -z "$entry" ]] && continue
        # rewritten_entries format is source:target:flags:excludes_b64
        # Extract source and target (first two fields)
    ...
    if [[ "$dry_run" != "true" ]]; then
        ...
        # Extract POSIX-compatible settings from zsh config (Mac users)
        if ! _cai_import_zsh_settings "$ctx" "$volume"; then
            _import_warn "Failed to extract zsh settings"
        fi
    else
        ...
        _import_step "[dry-run] Would extract POSIX-compatible settings from zsh config"
    fi
...
# ==============================================================================
# Zsh config extraction for Mac users
# ==============================================================================

# Extract POSIX-compatible settings from zsh config files for bash sourcing
# Extracts: simple exports, simple aliases (no zsh-specific syntax)
# Skips: setopt, autoload, compdef, bindkey, zle, zstyle, typeset -g, etc.
#
# Arguments: $1 = context, $2 = volume
# Returns: 0 on success (including when no zsh files exist), 1 on failure
_cai_import_zsh_settings() {
    ...
    # Check if any zsh config files exist on host
    local has_zsh_files=false
    local zsh_files=()
    ...
    if [[ "$has_zsh_files" != "true" ]]; then
        _import_info "No zsh config files found; skipping zsh settings extraction"
        return 0
    fi

    _import_step "Extracting POSIX-compatible settings from zsh config..."

    # Create temp file for extracted settings
    local tmp_extracted
    tmp_extracted=$(mktemp 2>/dev/null) || {
        _import_error "Failed to create temp file for zsh extraction"
        return 1
    }

    # Write header
    cat > "$tmp_extracted" << 'HEADER'
# POSIX-compatible settings extracted from zsh config files
# Generated by ContainAI import - do not edit manually
# Source: ~/.zshenv, ~/.zprofile, ~/.zshrc
#
# Only exports and simple aliases are extracted.
# Zsh-specific syntax (setopt, autoload, compdef, etc.) is skipped.

HEADER

    # Extract from each file in zsh startup order
    local file
    for file in "${zsh_files[@]}"; do
        printf '\n# From %s\n' "${file##*/}" >> "$tmp_extracted"

        # Use awk to extract POSIX-compatible lines:
        # - Lines starting with 'export ' followed by VAR=value
        # - Lines starting with 'alias ' followed by name='value' or name="value"
        # - Convert path+=('dir') to export PATH="dir:$PATH"
        # Skip:
        # - Comments, empty lines
        # - setopt, unsetopt, autoload, compdef, bindkey, zle, zstyle
        # - typeset, declare, local (when used for special zsh features)
        # - functions, source/., if/for/while blocks
        # - zsh-specific variables (ZDOTDIR, ZSH, fpath, etc.)
        awk '
            # Skip comments and empty lines
            /^[[:space:]]*#/ { next }
            /^[[:space:]]*$/ { next }

            # Skip zsh-specific commands
            /^[[:space:]]*(setopt|unsetopt|autoload|compdef|bindkey|zle|zstyle|typeset|declare|local|emulate|zmodload|add-zsh-hook)/ { next }

            # Skip function definitions
            /^[[:space:]]*function[[:space:]]/ { next }
            /^[[:space:]]*[a-zA-Z_][a-zA-Z0-9_]*[[:space:]]*\(\)/ { next }

            # Skip source/. commands (may source zsh-specific files)
            /^[[:space:]]*(source|\.)/ { next }

            # Skip control structures
            /^[[:space:]]*(if|then|else|elif|fi|for|do|done|while|until|case|esac|select)\b/ { next }

            # Convert path+=(...) to bash PATH exports
            # Handles: path+=(/foo) path+=("/foo") path+=('\''foo'\'')
            /^[[:space:]]*path\+=\(/ {
                line = $0
                # Remove leading whitespace and path+=(
                gsub(/^[[:space:]]*path\+=\(/, "", line)
                # Remove trailing )
                gsub(/\)[[:space:]]*$/, "", line)
                # Split on space and process each entry
            ...
            # Skip other zsh array syntax (fpath+=, cdpath+=, mailpath+=)
            /^[[:space:]]*(fpath|cdpath|mailpath)\+?=/ { next }

            # Extract simple exports: export VAR=value or export VAR="value"
            # Allow both uppercase and lowercase var names (http_proxy, etc.)
            # Denylist zsh-specific variables
            /^[[:space:]]*export[[:space:]]+[A-Za-z_][A-Za-z0-9_]*=/ {
                # Skip zsh-specific variables
                if (/export[[:space:]]+(ZDOTDIR|ZSH|ZSH_THEME|ZSH_CUSTOM|ZSH_CACHE_DIR|ZSH_COMPDUMP|FPATH|fpath|path|cdpath|mailpath)=/) next
                # Skip if it references zsh-specific vars
                if (/\$path\b|\$fpath\b|\$ZDOTDIR\b|\$ZSH\b/) next
                print
                next
            }

            # Extract simple aliases: alias name='"'"'..'"'"' or alias name="..."
            # Skip global aliases (-g flag), suffix aliases (-s flag)
            /^[[:space:]]*alias[[:space:]]+-[gs]/ { next }
            /^[[:space:]]*alias[[:space:]]+[a-zA-Z_][a-zA-Z0-9_-]*=/ {
                # Skip if it uses zsh-specific syntax in the value
                if (/=\$\(/ || /\$\{[^}]*:-/ || /\$\{[^}]*:=/) next
                print
                next
            }
    ...
    if ! DOCKER_CONTEXT= DOCKER_HOST= "${docker_cmd[@]}" run --rm --network=none --user 0:0 \
        ...
        "$_CAI_RSYNC_IMAGE" -c '
            ...
            if [ -L /target/shell/zsh-imported.sh ]; then
                echo "ERROR: /target/shell/zsh-imported.sh is a symlink" >&2
                exit 1
            fi

            cp /source/.zsh-imported.sh /target/shell/zsh-imported.sh
            chown 1000:1000 /target/shell/zsh-imported.sh
            chmod 644 /target/shell/zsh-imported.sh
        '; then
        rm -f "$tmp_extracted"
        _import_error "Failed to write zsh settings to volume"
        return 1
    fi

    rm -f "$tmp_extracted"

    _import_success "Extracted $line_count POSIX-compatible settings from zsh config"
    return 0
}
