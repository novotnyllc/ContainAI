    ...
    TextWriter standardError) : IDevcontainerSysboxVerificationService
{
    public async Task<int> VerifySysboxAsync(CancellationToken cancellationToken)
    {
        ...
        await DevcontainerSysboxCheckSuite.WriteHeaderAsync(standardOutput).ConfigureAwait(false);

        if (await DevcontainerSysboxCheckSuite.CheckSysboxFsAsync(processHelpers, standardOutput, cancellationToken).ConfigureAwait(false))
        {
            sysboxfsFound = true;
            passed++;
        }

        if (await DevcontainerSysboxCheckSuite.CheckUidMappingAsync(processHelpers, standardOutput, cancellationToken).ConfigureAwait(false))
        {
            passed++;
        }

        if (await DevcontainerSysboxCheckSuite.CheckNestedUserNamespaceAsync(processHelpers, standardOutput, cancellationToken).ConfigureAwait(false))
        {
            passed++;
        }

        if (await DevcontainerSysboxCheckSuite.CheckCapabilitiesAsync(processHelpers, standardOutput, cancellationToken).ConfigureAwait(false))
        {
            passed++;
        }
