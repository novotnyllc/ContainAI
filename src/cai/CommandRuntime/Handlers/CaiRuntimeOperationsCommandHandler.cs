...
internal sealed class CaiRuntimeOperationsCommandHandler
{
    private readonly CaiOperationsService operationsService;
    private readonly Func<DockerCommandOptions, CancellationToken, Task<int>> dockerRunner;

    public CaiRuntimeOperationsCommandHandler(CaiOperationsService service)
    {
        operationsService = service ?? throw new ArgumentNullException(nameof(service));
        dockerRunner = CaiOperationsService.RunDockerAsync;
    }

    public Task<int> RunDockerAsync(DockerCommandOptions options, CancellationToken cancellationToken)
        => dockerRunner(options, cancellationToken);
