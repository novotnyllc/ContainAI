...
internal sealed class CaiRuntimeConfigCommandHandler
{
    private readonly CaiConfigManifestService configManifestService;

    public CaiRuntimeConfigCommandHandler(CaiConfigManifestService service)
        => configManifestService = service ?? throw new ArgumentNullException(nameof(service));
